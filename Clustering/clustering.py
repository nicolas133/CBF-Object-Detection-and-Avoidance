...
class Clustering_Shaping:
    ...
    def __init__(self):
        ...
        rospy.Subscriber("/point_cloud", PointCloud, pcl_callback, queue_size=1)
    ...
    def bounding_box(cloud, clusters):
        for i in clusters:
            cluster_points = []
            cluster_points_x = []
            cluster_points_y = []
           ...
           # rectangle = np.array([[min_x, min_y], [max_x, min_y], [max_x, max_y], [min_x, max_y]])##npt used to print anymore
            ...
            self.wrapped_obj_pub(wrapped_objs)
