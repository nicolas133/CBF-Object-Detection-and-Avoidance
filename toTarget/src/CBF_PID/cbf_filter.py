...
class CBF_safetyFilter:
    ...
    def optimizer(self):
        ...
        circular_obstacle = obstacle(R=self.radius_list[0],x=[self.centroid_xlist[0], self.centroid_ylist[0]])
        ...
        # Compute CBF h-value
        hvalue= circular_obstacle.h_of_x() 
        ...
        print("this works")
        a = array([0,self.current_V* (-2*np.cos(current_SA)*(self.centroid_xlist[0]) + -2*np.sin(current_SA)*(self.centroid_ylist[0]))])

        #a = -a
        print("Full Time Interval :" + str(a))
        print("a: ", str(a))
