...
class controllerNode:
	...
	self.finished = False
        ...
        print("initial twist: ", str(self.twist_msg))
	...
	# define current velocity
        current_vel_x = self.twist_msg.linear.x
	current_vel_y = self.twist_msg.linear.y
	print("cur xv: ", str(current_vel_x))
	print("cur yv: ", str(current_vel_y))

	# define current steering angle
        self.current_steering_angle = self.twist_msg.angular.z
	...
	# set initial x pos to 0
        self.current_pose = self.pose_msg.pose.pose.position
        self.current_ort = self.pose_msg.pose.pose.orientation
	...
	# print current waypoint for debugging purposes
        ...
        x_coord = self.current_pose.x
	y_coord = self.current_pose.y
        z_coord = self.current_pose.z
	
	# print current x and y values
        print("AMCL X-coord: " + str(x_coord))
        print("AMCL Y-coord: " + str(y_coord))

        #print(self.odom_msg.pose.pose.position.x)
        #print(self.odom_msg.pose.pose.position.y)
	...
	#new_vel.linear.y = self.current_vel_y + self.Speed_PID.pid_compute(y_error, self.current_vel_y)
        new_vel.angular.z = (self.current_steering_angle) + self.Speed_PID.pid_compute(desired_angle, theta)
